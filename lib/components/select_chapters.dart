...
class _ChapterSelectionScreenState extends State<ChapterSelectionScreen> {
  ...
  List<List<String>> finalSelectedChapters = [];
  List<String> selectedChapterTitles = [];
    ...
    return Scaffold(
      ...
      body: ListView(
        children: subjectProv.filteredSubjects
            ...
            .where((subject) => subject.title != "Custom Topic")
            .map((subject) {
          return Padding(
            padding:
                ...
                onChanged: (isSelected) {
                  setState(() {
                    if (isSelected == true) {
                      ...
                      selectedChapterTitles.add(subject.title);
                    } else {
                      selectedChapters.remove(subject.id);
                      selectedChapterTitles.remove(subject.title);
                    }
                  });
                },
              ),
      ...
      bottomNavigationBar: Padding(
        ...
        child: Row(
          ...
          children: [
            ...
            ElevatedButton(
              onPressed: () {
                finalSelectedChapters.add(selectedChapters);
                finalSelectedChapters.add(selectedChapterTitles);
                print("Judul chapter: $selectedChapterTitles");

                Navigator.of(context)
                    .pop(finalSelectedChapters); // Kembali dengan pilihan
              },
              style: ElevatedButton.styleFrom(
                backgroundColor: const Color(0xFF1C88BF),
                padding: const EdgeInsets.symmetric(
